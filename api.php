<?php include("includes/connection.php");
	 ...
	 if(isset($_GET['home']))
	  {
	    ...
	    LEFT JOIN tbl_category ON tbl_books.cat_id= tbl_category.cid
	    LEFT JOIN tbl_author ON tbl_books.aid= tbl_author.author_id   
	    ORDER BY tbl_books.id DESC LIMIT 8";
      ...
      LEFT JOIN tbl_category ON tbl_books.cat_id= tbl_category.cid
      LEFT JOIN tbl_author ON tbl_books.aid= tbl_author.author_id   
      ORDER BY tbl_books.book_views DESC,tbl_books.total_rate DESC LIMIT 4";
    ...
    else if(isset($_GET['cat_list']))
 	{
		...
		$query="SELECT cid,category_name,cat_image FROM tbl_category ORDER BY tbl_category.".$cat_order."";
			...
			//Comments
		      ...
		      if($result3->num_rows > 0)
		      {
		      		while ($row_comments=mysqli_fetch_array($result3)) {
 		      	        $row3['id'] = $row_comments['id'];
		 		      	$row3['book_id'] = $row_comments['book_id'];
		 		      	$row3['user_name'] = $row_comments['user_name'];
		 		      	$row3['user_image'] = $row_comments['user_image'];
 		 		      	$row3['comment_text'] = $row_comments['comment_text'];
		 		      	$row3['dt_rate'] = date('d-M-Y G:i:s A',strtotime($row_comments['dt_rate']));
